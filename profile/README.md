...
옮겨 온 프로젝트는 아래 기준을 맞춘 뒤에 공개합니다.

| | 내용 |
| --- | --- |
| **실행 가능** | README에 적힌 명령만으로 처음부터 실행됩니다. |
| **살아 있는 데이터** | 예시 데이터를 실행 시점 기준으로 만들어, 날짜가 지나 비어 보이지 않게 합니다. |
| **자격 증명 분리** | 접속 정보는 저장소에 올리지 않고 설정 파일과 환경 변수로 분리합니다. |
| **버그 수정 기록** | 당시 해결하지 못한 문제는 원인을 찾아 고치고, 무엇을 왜 바꿨는지 README에 남깁니다. |
| **원본 이력 보존** | 원래 저장소의 커밋 이력을 그대로 가져온 뒤 정리 커밋을 얹습니다. |
...
> 과정에서 만든 나머지 프로젝트도 같은 기준으로 하나씩 옮겨 올 예정입니다.
